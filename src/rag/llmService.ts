...
ПРИКЛАД ВІДПОВІДІ:
"Ось жарт для тебе, {userName}: [жарт з контексту] 😄 Хочеш ще один?"
...
`);
    ...
    async generateGreeting(userName?: string): Promise<string> {
        const prompt = PromptTemplate.fromTemplate(`
Ти - дружелюбний бот-жартівник. Привітайся з користувачем українською мовою.

{userContext}

Зроби привітання коротким, дружелюбним та запитай як користувача звуть (якщо ім'я невідоме).
Додай емодзі.
...
`);
        ...
        const userContext = userName
            ? `Користувача звуть: ${userName}`
            : "Ім'я користувача невідоме";
